...
const read_from_parts = (start, end, parts) => {
  ...
  if (parts.length === 1 && parts[0][0] === start && parts[0][1] === end) {
	...
	for (const [ open, close, part ] of parts) {
		let _end = close > end ? end - open : close - open
		if (open < start) {
			concatable.push(part.subarray(start - open, _end))
		} else {
			concatable.push(part.subarray(0, _end))
		}
	}
	return concat(concatable, end - start)
}
...
class MemoryByteArray extends ByteArray {
	constructor (array) {
		const instructions = [ array.length, ...array.map(part => part.byteLength) ]
		super([ null, instructions, null ])
		this._inmem = array
	}
	static from (array) {
...
class ByteArrayProof extends Proof {
	static build (byte_array, hasher) {
		return this.from(byte_array.map(part => hasher.digest(part).digest))
	}
}

export { ByteArrayProof, Proof, MemoryByteArray, Input, Centerless, ByteArray }
